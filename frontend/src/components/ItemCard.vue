<template>
  <article class="item" :class="{ clickable: !editable, 'no-hover': isItem }" @onclick="!editable && emit('click', $event)">
    ...
    <div v-if="!editable && quantity !== undefined" class="item__controls">
      <button type="button" class="item__controls__button" :disabled="quantity < 1" @click="$emit('qtyChange', -1)">-</button>
      <span class="item__controls__qty">{{ quantity }}</span>
      <button type="button" class="item__controls__button" @click="$emit('qtyChange', 1)">+</button>
    </div>
